...
Public Class FormLaporanPengembalian
    Dim nobln As String
    Private Sub FormLaporanPengembalian_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tampildatablnthn()
    End Sub
    Sub tampilsemua()
        Call Koneksi()
        Da = New SqlDataAdapter("select * from tbl_kembali", Conn)
    ...
    Sub tampilblnthn()
        If ComboBox1.Text <> "" And ComboBox2.Text <> "" Then
            Try
                ...
                Dim Str As String = "select * from tbl_kembali where month(tglkembali) = '" & nobln & "' and year (tglkembali) = '" & ComboBox2.Text & "' order by tglkembali"
        ...
        ElseIf ComboBox1.Text = "" And ComboBox2.Text <> "" Then
            Try
                ...
                Call Koneksi()
                Ds = New DataSet
                Dim Str As String = "select * from tbl_kembali where  year (tglkembali) = '" & ComboBox2.Text & "' order by tglkembali"
                Da = New SqlDataAdapter(Str, Conn)
                Da.Fill(Ds)
